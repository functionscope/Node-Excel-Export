...
var sheetsBack = '</sheets><calcPr calcId="144525"/></workbook>';
...
function generateMultiSheets(configs, xlsx) {
	...
	configs.forEach(function(config) {
		config.name = 'sheet'+i;
		i++;
		console.log('generateMultiSheets::', config);
		generateSheet(config, xlsx);
	});
}
...
function generateRel(configs,xlsx) {
	...
	workbook += relBack;
	console.log('xl/_rels/workbook.xml.rel,', workbook);
...
function generateWorkbook(configs,xlsx) {
	...
	var i = 1;	
	configs.forEach( function(config) {
		workbook += '<sheet name="'+ (config.name || 'sheet') + '" sheetId="' + i +'" r:id="rId' + i + '"/>';
		i++;
	});
	workbook += sheetsBack;
	console.log('xl/workbook.xml,', workbook);
...
exports.execute = function(config) {
	...
	generateSharedStringsFile(sharedStringsXml, xlsx);
...
function generateSheet(config, xlsx) {
	...
	console.log('config.name::', config.fileName);
	//	var sheet = xlsx.file(config.name, '\r\n');
